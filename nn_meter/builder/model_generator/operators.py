...
def dwconv(input_shape, config = None):
    return keras.layers.DepthwiseConv2D(config['kernel_size'], padding=config['padding']), input_shape, False
...
def conv(input_shape, config = None):
    return keras.layers.Conv2D(input_shape[2], config['kernel_size'], padding=config['padding']), input_shape, False
