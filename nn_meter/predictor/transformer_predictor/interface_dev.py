        ...
        for stage_idx, channel in enumerate(block_config[1]):
            ...
            if name == "conv":
                ...
                for i in range(block_config[2][stage_idx]):
                    ...
                    tpy = 0
                    if ds == "ds":
                        tpy += self.predictor[f"transds_{layer_hw}_{cin}_{cout}_{s}_6"]
                        print(f"transds_{layer_hw}_{cin}_{cout}_{s}_6")
                        layer_hw = stage_hwout
                    tpy += self.predictor[f'transattn_{layer_hw}_{cout}_{act}_{v}_{"ln" if self.layer_norm else "bn"}']
                    print(f'transattn_{layer_hw}_{cout}_{act}_{v}_{"ln" if self.layer_norm else "bn"}')
                    tpy += self.predictor[f'transffn_{layer_hw}_{cout}_{exp}_{act}_{"ln" if self.layer_norm else "bn"}']
                    print(f'transffn_{layer_hw}_{cout}_{exp}_{act}_{"ln" if self.layer_norm else "bn"}')
