...
def build_models(key, name, hw, cin, cout, exp, s, act, ks = None, v = None, ds = None, use_se = None, ds_exp = None):
    ...
    if name == "conv": # conv
        ...
        inputs = keras.Input(shape=[hw, hw, cin], batch_size=1)
        output = transformer_ds(inputs, cout, ds_exp=ds_exp, stride=s)
    
    elif name == "transattn": # transformer attention layer
        inputs = keras.Input(shape=[hw * hw, cin], batch_size=1)
        output = transformer_attn(inputs, cout, v_scale=v, name=name, layer_norm=layer_norm)
    
    elif name == "transffn": # transformer attention layer
        inputs = keras.Input(shape=[hw * hw, cin], batch_size=1)
        output = transformer_ffn(inputs, cout, expansion_ratio=exp, name=name, layer_norm=layer_norm)
    ...
    converted_model = os.path.join(main_path, "trans_blocks", f"{key}.tflite")
                        ...
                        for s in [config['stride']]:
                            for v in config['v_scale']:
                                for ds_exp in config['downsample_expansion_ratio']:
                                    # transds
                                    new_key = f'transds_{hw}_{cin}_{cout}_{s}_{ds_exp}'
                                    if new_key not in lut_result:
                                        if lut_result_ref and new_key in lut_result_ref:
                                            print(new_key)
                                            continue
                                        else:
                                            build_models(new_key, "transds", hw, cin, cout, exp, s, act, v=v, ds=ds, ds_exp=ds_exp)
                                            lut_result[new_key] = [[hw, hw, cin]]

                                    # transattn
                                    new_key = f'transattn_{hw}_{cout}_{act}_{v}_{"ln" if layer_norm else "bn"}'
                                    if new_key not in lut_result:
                                        if lut_result_ref and new_key in lut_result_ref:
                                            print(new_key)
                                            continue
                                    ...
                                    # transffn
                                    new_key = f'transffn_{hw}_{cout}_{exp}_{act}_{"ln" if layer_norm else "bn"}'
                                    if new_key not in lut_result:
                                        if lut_result_ref and new_key in lut_result_ref:
                                            print(new_key)
                                            continue
        ...
        for cout in config["channel"]:
            ...
            for exp in config['expansion_ratio']:
                for act in [ACT]:
                    for v in config['v_scale']:
                        # transattn
                        new_key = f'transattn_{hw}_{cin}_{act}_{v}_{"ln" if layer_norm else "bn"}'
                        if new_key not in lut_result:
                            if lut_result_ref and new_key in lut_result_ref:
                                print(new_key)
                                continue
                        ...
                        # transffn
                        new_key = f'transffn_{hw}_{cin}_{exp}_{act}_{"ln" if layer_norm else "bn"}'
                        if new_key not in lut_result:
                            if lut_result_ref and new_key in lut_result_ref:
                                print(new_key)
                                continue
...
if __name__ == '__main__':
    ...
    with open(os.path.join(main_path, "results_pixel6", f"lut_{'ln' if layer_norm else 'bn'}_v6.json"), 'w') as fp:
        json.dump({"lut": res}, fp, indent=4)
        # json.dump(res, fp, indent=4)


# nohup python /data/data0/jiahang/nn-Meter/examples/test_transformer/build_lut/build_lut_attnffn.py ln > trans_ln_attnffn_lut_log.txt 2>&1 &
# [1] 6150
