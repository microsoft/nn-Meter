...
def transformer_layer(inputs, channels, expansion_ratio, v_scale, stride, layer_index, name):
    if layer_index == 0:
        inputs = attention_downsampling(inputs, channels, downsampling=stride==2)
    ...
    nn = attention(inputs, channels, num_heads, key_dim, v_dim, 1, activation=ACT, name=name)
    nn = mlp(nn, expansion_ratio, activation=ACT, name=name)
