...
def dsconv(inputs, channel, act, strides, kernel_size, exp, use_se=False):
    ...
    if use_se:
        se_d = tf.keras.layers.GlobalAveragePooling2D()(nn)
        # print(se_d.shape)
...
def conv_layer(inputs, channel, expansion_ratio, kernel_size, stride, act=ACT):
    return dsconv(inputs, channel, strides=stride, kernel_size=kernel_size, exp=expansion_ratio, act=act)