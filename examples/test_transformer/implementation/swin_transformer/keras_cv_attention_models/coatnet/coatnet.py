...
def mhsa_with_multi_head_relative_position_embedding(
    ...
    qkv = keras.layers.Dense(qk_out * 2 + out_shape, use_bias=False, name=name and name + "qkv_")(inputs)
...
def res_ffn(inputs, expansion=4, kernel_size=1, drop_rate=0, activation="gelu", name=""):
    ...
    nn = keras.layers.Dense(input_channel * expansion, use_bias=False, name=name + "1_")(preact)
    ...
    nn = keras.layers.Dense(input_channel, use_bias=False, name=name + "2_")(nn)
    nn = drop_block(nn, drop_rate=drop_rate, name=name)
    return keras.layers.Add()([inputs, nn])


def res_mhsa(inputs, output_channel, conv_short_cut=True, strides=1, head_dimension=32, drop_rate=0, activation="gelu", name="mhsa"):
...
def CoAtNet_co(input_shape=(224, 224, 3), expansion=4, num_classes=1000, activation="gelu", classifier_activation="softmax", **kwargs):
    num_blocks = [2, 3, 5, 2]
