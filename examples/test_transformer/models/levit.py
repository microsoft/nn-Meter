...
from .levit_layers import batchnorm_with_activation, conv2d_no_bias, activation_by_name,layer_norm
...
def mhsa_with_multi_head_position(inputs, output_dim, num_heads, key_dim, attn_ratio, activation="hard_swish", name=""):
    ...
    qkv = tf.reshape(qkv, (-1, blocks, num_heads, qkv_dim // num_heads), name=name and name + "reshape")
    qkv = tf.transpose(qkv, perm=[0, 2, 1, 3], name=name and name + "Lambda")
    qq, kk, vv = tf.split(qkv, [key_dim, key_dim, key_dim * attn_ratio], axis=-1, name=name and name + "split")
...
def mhsa_with_multi_head_position_windows_layer_norm(inputs, output_dim, num_heads, key_dim, v_dim, window_size, activation="hard_swish", name=""):
    ...
    qkv_dim = int(2 * embed_dim + embed_dim_v)
    qkv = keras.layers.Dense(qkv_dim, use_bias=False, name=name + "qkv")(inputs)
    qkv = activation_by_name(qkv, activation=None, name=name+'_act')
    qkv = tf.reshape(qkv, (-1, ww, num_heads, int(qkv_dim//num_heads)), name=name and name + "reshape")
    qkv = tf.transpose(qkv, perm=[0, 2, 1, 3], name=name and name + "Lambda")
    qq, kk, vv = tf.split(qkv, [key_dim, key_dim, v_dim], axis=-1, name=name and name + "split")
...
def mhsa_with_multi_head_position_and_strides(inputs, output_dim, num_heads, key_dim, attn_ratio=2, strides=1, activation="hard_swish", name=""):
    ...
    if strides != 1:
        ...
        qq = tf.reshape(inputs, (-1, width, width, channel), name=name and name + "reshape")[:, ::strides, ::strides, :]
        qq = tf.reshape(qq, [-1, qq.shape[1] * qq.shape[2], channel], name=name and name + "reshape2")
    else:
        qq = inputs
    ...
    qq = tf.reshape(qq, [-1, qq.shape[1], num_heads, key_dim], name=name and name + "reshape3")
    qq = tf.transpose(qq, [0, 2, 1, 3], name=name and name + "t")
    ...
    kv = tf.reshape(kv, (-1, blocks, num_heads, kv_dim // num_heads), name=name and name + "reshape4")
    kv = tf.transpose(kv, perm=[0, 2, 1, 3], name=name and name + "t")
    kk, vv = tf.split(kv, [key_dim, key_dim * attn_ratio], axis=-1, name=name and name + "split")
    return scaled_dot_product_attention(qq, kk, vv, key_dim, attn_ratio, output_dim=output_dim, activation=activation, name=name)
...
def attention_mlp_stack(inputs, out_channel, num_heads, depth, key_dim, attn_ratio, mlp_ratio, strides, stack_drop=0, activation="hard_swish", name=""):
    ...
    for ii in range(depth):
        ...
        nn = res_mhsa_with_multi_head_position(nn, embed_dim, num_heads, key_dim, attn_ratio, drop_rate, activation=activation, name=block_name + "mhsa_")
        if mlp_ratio > 0:
            nn = res_mlp_block(nn, mlp_ratio, drop_rate, activation=activation, name=block_name + "ffn_")
    if embed_dim != out_channel:
        block_name = name + "downsample_"
        ds_num_heads = embed_dim // key_dim
        ds_attn_ratio = attn_ratio * strides
        nn = mhsa_with_multi_head_position_and_strides(nn, out_channel, ds_num_heads, key_dim, ds_attn_ratio, strides, name=block_name+ "ds_mhsa_")
        if mlp_ratio > 0:
            nn = res_mlp_block(nn, mlp_ratio, drop_rate, activation=activation, name=block_name + "ds_ffn_")
    return nn


def attention_downsampling(inputs, channels, layer_norm):
    B, L, C = inputs.shape
    H = W = int(L**0.5)
    ...
    if layer_norm:
        nn = layer_norm(nn)
    nn = tf.keras.layers.Dense(channels)(nn)
    if not layer_norm:
...
class Model(keras.Model):
    ...
    def call(self, inputs):
        x = mhsa_with_multi_head_position_windows(
            inputs, output_dim=128, num_heads=4,
            key_dim=32, v_dim=32, window_size=7, name = "123")
        return x
